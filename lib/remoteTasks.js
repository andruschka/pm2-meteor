...
(function() {
  ...
  module.exports = {
    ...
    extractTarBall: function(session, pm2mConf, done) {
      return session.execute("cd " + (getAppLocation(pm2mConf)) + " && tar -xf " + _settings.bundleTarName, {}, function(err, code, logs) {
        if (err) {
          return done(err);
        } else {
          return done();
    ...
    reloadApp: function(session, pm2mConf, done) {
      return session.execute("cd " + (getAppLocation(pm2mConf)) + " && pm2 startOrRestart " + _settings.pm2EnvConfigName, {}, function(err, code, logs) {
        if (err) {
          return done(err);
        } else {
          if (logs.stderr) {
