...
(function() {
  ...
  module.exports = {
    ...
    prepareHost: function(session, pm2mConf, done) {
      var cmd;
      cmd = cmdString(pm2mConf, "sudo mkdir -p " + (path.join(getAppLocation(pm2mConf), _settings.backupDir)));
      return session.execute(cmd, {}, function(err, code, logs) {
        if (err) {
          return done(err);
        } else {
    ...
    extractTarBall: function(session, pm2mConf, done) {
      var cmd;
      cmd = cmdString(pm2mConf, "sudo cd " + (getAppLocation(pm2mConf)) + " && rm -rf " + _settings.bundleName + " && tar -xf " + _settings.bundleTarName);
      return session.execute(cmd, {}, function(err, code, logs) {
        if (err) {
          return done(err);
        } else {
    ...
    installBundleDeps: function(session, pm2mConf, done) {
      ...
      cmd = cmdString(pm2mConf, "sudo cd " + serverLocation + " && node --version && npm i .");
      return session.execute(cmd, {}, function(err, code, logs) {
        if (err) {
          return done(err);
        } else {
    ...
    startApp: function(session, pm2mConf, done) {
      var cmd;
      cmd = cmdString(pm2mConf, "sudo cd " + (getAppLocation(pm2mConf)) + " && pm2 start " + _settings.pm2EnvConfigName);
      return session.execute(cmd, {}, function(err, code, logs) {
        if (err) {
          return done(err);
        } else {
    ...
    stopApp: function(session, pm2mConf, done) {
      var cmd;
      cmd = cmdString(pm2mConf, "sudo cd " + (getAppLocation(pm2mConf)) + " && pm2 stop " + _settings.pm2EnvConfigName);
      return session.execute(cmd, {}, function(err, code, logs) {
        if (err) {
          return done(err);
        } else {
    ...
    backupLastTar: function(session, pm2mConf, done) {
      var cmd;
      cmd = cmdString(pm2mConf, "sudo cd " + (getAppLocation(pm2mConf)) + " && mv " + _settings.bundleTarName + " backup/ 2>/dev/null");
      return session.execute(cmd, {}, function(err, code, logs) {
        if (err) {
          return done();
        } else {
    ...
    softReloadApp: function(session, pm2mConf, done) {
      var cmd;
      cmd = cmdString(pm2mConf, "sudo cd " + (getAppLocation(pm2mConf)) + " && pm2 startOrReload " + _settings.pm2EnvConfigName);
      return session.execute(cmd, {}, function(err, code, logs) {
        if (err) {
          return done(err);
        } else {
    ...
    hardReloadApp: function(session, pm2mConf, done) {
      var cmd1;
      cmd1 = cmdString(pm2mConf, "sudo cd " + (getAppLocation(pm2mConf)) + " && pm2 delete " + pm2mConf.appName);
    ...
    deleteAppFolder: function(session, pm2mConf, done) {
      var cmd;
      cmd = cmdString(pm2mConf, "sudo rm -rf " + (getAppLocation(pm2mConf)));
      return session.execute(cmd, {}, function(err, code, logs) {
        if (err) {
          return done(err);
        } else {
    ...
    revertToBackup: function(session, pm2mConf, done) {
      ...
      cmd = cmdString(pm2mConf, "sudo mv " + (path.join(backupLocation, _settings.bundleTarName)) + " " + (path.join(appLocation, _settings.bundleTarName)));
